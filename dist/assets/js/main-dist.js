...
var _common = {
  ...
  //點擊空白處
  clickBlank: function clickBlank(page) {
  ...
  // 登入提示視窗
  remindLogin: function remindLogin() {
    var intervalTime = $('body').attr('data-login-remind') == '' ? false : Number($('body').attr('data-login-remind'));
  ...
  animateInview: function animateInview() {
    $('[data-animate]').each(function () {
      var obj = $(this);
      var inviewObj = obj.attr('data-anchor-obj') || this;
      var animateName = obj.attr('data-animate');
      var duration = obj.attr('data-duration') || '1s';
      var easing = obj.attr('data-easing') || 'linear';
  ...
  //footer進入畫面時
  footerInview: function footerInview() {
    var now_page = $('body').attr('class');
    //有裝飾物的頁面
    var hasDecor = ['news', 'search_result', 'product', 'product_list', 'qa', 'about', 'contact', 'login'];
  ...
  //帳號流程類共用
  accountCommon: function accountCommon(data) {
    ...
    $(container).on('click', function () {
      var stepCount = $('.step-content').length;
      var Route = $(this).find('.close-btn').attr('data-ajax-route');
      var Container = $(this).find('.close-btn').attr('data-ajax-container');
      var Callback = $(this).find('.close-btn').attr('data-ajax-callback');
    ...
    $(container).find('.close-btn').on('click', function (e) {
      if ($(window).outerWidth() <= 991) {
        e.stopPropagation();
        var stepCount = $('.step-content').length;
        var Route = $(this).attr('data-ajax-route');
    ...
    function moveCloseBtnDom() {
      var closeBtn = $(container).find('.close-btn');
      if ($(window).outerWidth() <= 767) {
  ...
  accountStepChange: function accountStepChange(stepCount, type) {
    var step = $('.step-circle').attr('data-step');
    var next = type == 'back' ? Number(step) - 1 : Number(step) + 1;
  ...
  //步驟function
  stepFunction: function stepFunction(step) {
    switch (step) {
      case 2:
        countDown();
  ...
  //視窗卷軸控制
  scrollbarController: function scrollbarController(pagename) {
    var scrollElement = $(window);
    var nav_h = $('nav:not(.sticky)').innerHeight() + 200;
    var after = 0;
    ...
    scrollElement.on('scroll', function () {
      ...
      switch (pagename) {
        ...
        case 'news':
          if (window_w > 767) {
            var newsTypeWidth = $('.news-type').innerWidth();
            var typeSelectHeight = $('.type-select').outerHeight(true);
            var newsListHeight = $('.news-list').outerHeight(true);
            var newsTypePosLeft = $('.news-type').position().left;
            var hrOffset = $('.container > .custom-hr').offset().top;
        ...
        case 'qa':
          if (window_w > 1024) {
            ...
            var qaPaddingBottom = Number($('.qa-block').css('paddingBottom').replace('px', ''));
            var _viewHeight = $(window).outerHeight();
            var typeSelectOffsetTop = 470;
            var _typeSelectHeight = $('.qa-type .type-select').height();
            var typeSelectOffsetBottom = _viewHeight - typeSelectOffsetTop - _typeSelectHeight;
            var stopPoint = footerOffsetTop - qaPaddingBottom - _viewHeight;
        ...
        case 'product_search':
          if ($('.result-wrap').hasClass('search-after')) {
            var headerMarginB = Number($('.result-wrap .header').css('margin-bottom').replace('px', ''));
            var listOffsetTop = $('.product-list').offset().top;
            var keywordContainer = $('.search-keyword-container');
            var emptyBox = '<div class="empty-box" style="width: 100%;height: ' + keywordContainer.outerHeight(true) + 'px;margin: 0 auto;"></div>';
      ...
      if ($('footer').length > 0) {
        ...
        var footerTop = $('.footer-top').innerHeight();
        var footerBottomOffset = $('.footer-bottom').offset().top;
  ...
  switchTabs: function switchTabs(fadeOut) {
    $('.tab-btn').on('click', function (e) {
      ...
      if (_click.hasClass('active')) {
        ...
        var tabsDataID = nav_tabs.attr('data-id');
        var tabsID = '#' + tabsDataID;
        var tab_content = $(tabsID);
        var page = _click.attr('data-page');
        var tabpaneID = '#' + page;
  ...
  //抓swiper
  getMySwiper: function getMySwiper(name) {
  ...
  //判斷輪播數量
  getSlidesCount: function getSlidesCount(swiper) {
  ...
  //主圖輪播
  bannerSlider: function bannerSlider(slider) {
    var activeSlide = void 0;
    var swiper = slider.$el ? slider.$el : $(slider);
    var bannerSlider = $(swiper).parents('.mainBanner-slider');
    var textarea = bannerSlider.find('.slider-textarea');
    var slideCount = slider.slides ? slider.slides.length - 2 : 1;
    if (slideCount <= 1) {
    ...
    var day = activeSlide.attr('data-day');
    var monthYear = activeSlide.attr('data-month-year');
    var title = activeSlide.attr('data-title');
    var description = activeSlide.attr('data-description');
    var ajaxHref = activeSlide.attr('data-href');
  ...
  //打開Ajax
  openAjax: function openAjax(route, container, callback, obj) {
    var contain = container ? container : '.remind-login-lbox';
    var func = callback ? callback : '_ajaxCallback.remindlogin';
    $.ajax4({
      Route: route,
      Container: contain,
      Block: '',
  ...
  //關閉Ajax
  closeAjax: function closeAjax(container) {
  ...
  ajaxCommon: function ajaxCommon(data) {
    var container = data.Container;
    $('nav.sticky').hide();
    ...
    switch (container) {
      case '.preview-wrap':
        ...
        var _previewBefore = function _previewBefore() {
          var form = $('.remark-block .content').clone();
          var paperName = [$('.toolbar-top .product-name span:nth-child(2)').attr('data-default'), 'Attached file-1', 'Attached file-2'];
          $('.drawing-paper.paper-initialize').each(function (index, el) {
            var page = $(el).attr('data-page');
            ...
            var previewBlock = '<div class="preview-block"><div class="custom-name"><span>Name</span><span>' + paperName[index] + '</span></div><div class="preview-img"><img src="' + imgBase64[index] + '" /></div></div>';
        ...
        var _formFormat = function _formFormat() {
          $('.form-block .form input').prop('disabled', true);
          $('.form-block .form .form-group').each(function (index, el) {
            var cancel = $(el).find('.cancel-option input').prop('checked');
            var option = $(el).find('.option-group label.checkbox input:checked');
            if (cancel == true) {
  ...
  //input 數量改變
  countHandler: function countHandler(ele) {
    //失去焦點時如果數量為空值則改為1
    ...
    $(ele).on('keyup', '.buy-count .count', function () {
      ...
      //庫存數量
      var inventories = Number($(this).parents('.buy-count').attr('data-inventories'));
      //只能輸入非零開頭的正整數
      var filter = this.value = this.value.replace(/\D/g, '') == '' ? '' : parseInt(this.value, 10);
      $(this).val(filter);
      //如果輸入數量大於庫存數量
    ...
    $(ele).on('click', '.buy-count .minus', function (e) {
      e.stopPropagation();
      //目前數量
      var count = Number($(this).parents('.buy-count').find('.count').val());
      if (count > 1) {
        count--;
  ...
  //Blazy基本
  blazyInit: function blazyInit(pagename) {
    var bLazy = new Blazy({
...
var _ajaxCallback = {
  //登入提醒視窗
  remindlogin: function remindlogin(data) {
    var container = data.Container;
    $('nav.sticky').hide();
    $(container).addClass('ajax_close');
    $(container).find('.modal-scroll-wrap').scrollbar({
  ...
  //一般訊息燈箱
  normalLightbox: function normalLightbox(data) {
    _common.ajaxCommon(data);
    _common.bodyLock('.modal-scroll-wrap');
    var lbox = data.Container;
    switch (lbox) {
  ...
  //首頁探索靈感
  homeInspire: function homeInspire(data) {
    var container = data.Container;
    function moveDecorObject() {
      var window_w = $(window).outerWidth();
      var object = $('.decor-layer');
      if (window_w <= 767) {
    ...
    $(container).find('.modal-scroll-wrap').scrollbar({
      ...
      "onScroll": function onScroll(y, x) {
        var contentOffset = $('.modal-content').position().top + 90;
        var window_w = $(window).outerWidth();
        if (window_w > 1024) {
  ...
  //最新消息內頁
  newsDetail: function newsDetail(data) {
    _common.ajaxCommon(data);
    ...
    function swiper() {
      if ($('.mainBanner-slider').length > 0) {
        ...
        var slider = _common.getMySwiper('detail-banner-slider');
    ...
    //關閉按鈕dom移動
    function moveCloseBtnDom() {
      var btn = $('.news_detail .close-btn.ajax_close');
      if ($(window).outerWidth() <= 1024) {
  ...
  //編輯收藏
  editCollect: function editCollect(data) {
    var obj = data.ClickObj;
    var optionText = $(obj).parents('.edit').siblings('label').text();
    _common.ajaxCommon(data);
    _common.bodyLock('.modal-scroll-wrap');
  ...
  guide: function guide(data) {
    ...
    function swiper() {
      if ($('.mainBanner-slider').length > 0) {
        ...
        var slider = _common.getMySwiper('guide-banner-slider');
        _common.bannerSlider(slider);
        slider.on('slideChangeTransitionStart', function () {
          _common.bannerSlider(slider);
        });
  ...
  //隱私權政策
  privacyPolicy: function privacyPolicy(data) {
    _common.ajaxCommon(data);
    _common.bodyLock('.modal-scroll-wrap');
  ...
  //帳戶流程類
  accountStep: function accountStep(data) {
    ...
    if (data.Container === '.register-lbox' || data.Container === '.forgetps-lbox') {
      $('.bookmark-switch [data-bookmark]').on('click', function () {
        var bookmark = $(this);
        var type = bookmark.attr('data-bookmark');
  ...
  //更換大頭貼
  changePhoto: function changePhoto(data) {
    _common.ajaxCommon(data);
    _common.bodyLock('.modal-scroll-wrap');
    var memberPhoto = $('.welcome .member-photo');
    var memberPhotoUrl = memberPhoto.css('background-image');
    ...
    $('.modal-btn.save').on('click', function () {
      var changeAfterUrl = $('.member-photo.edit').css('background-image');
      var route = $(this).attr('data-ajax-route');
      var container = $(this).attr('data-ajax-container');
      var callback = $(this).attr('data-ajax-callback');
  ...
  changePhotoSuccess: function changePhotoSuccess(data) {
    _common.ajaxCommon(data);
  ...
  //會員條款
  memberRegulation: function memberRegulation(data) {
    _common.ajaxCommon(data);
    _common.bodyLock('.modal-scroll-wrap');
  ...
  //GDPR
  gdprLightbox: function gdprLightbox(data) {
    _common.ajaxCommon(data);
    _common.bodyLock('.modal-scroll-wrap');
...
var _menu = {
  swiper: function swiper() {
    function changeSlide(slider) {
      var activeSlide = void 0;
      var swiper = slider.$el ? slider.$el : $(slider);
      var textBox = $('.menu-photo .text-box');
      //如果Swiper沒有啟動
      if (swiper.attr('data-swiper4-active') !== 'on') {
        activeSlide = swiper.find('.swiper-slide');
      } else {
      ...
      var line1 = activeSlide.attr('data-line1');
      var line2 = activeSlide.attr('data-line2');
    ...
    var menuSwiper = _common.getMySwiper('menu-banner');
  ...
  languageBtn: function languageBtn() {
...
var _home = {
  swiper: function swiper() {
    var inspiration_L = _common.getMySwiper('inspiration-left');
    var inspiration_R = _common.getMySwiper('inspiration-right');
    var newsSlider = _common.getMySwiper('home-news-slider');
  ...
  //探索區塊圖片寬度轉為百分比
  ImgWidthConvert: function ImgWidthConvert(img) {
    setTimeout(function () {
      //抓到圖片原始尺寸
      var pixel_w = $(img)[0].naturalWidth;
      var percent_w = Math.round(pixel_w / 5.5 * 1000) / 1000 >= 100 ? 100 : Math.round(pixel_w / 5.5 * 1000) / 1000;
  ...
  //關於我們切換
  aboutChange: function aboutChange() {
    //全域變數
    var autoplay = void 0;

    //切換
    function changeHandler(index) {
      var li = $('.closeToMing-wrap .about-menu li');
      var large = $('.closeToMing-wrap .large-wrap img');
      var small = $('.closeToMing-wrap .small-wrap .about-info');
    ...
    function autoPlay(time) {
      autoplay = setInterval(function () {
        var nowIndex = Number($('.closeToMing-wrap').attr('data-show-index')) == 2 ? -1 : Number($('.closeToMing-wrap').attr('data-show-index'));
    ...
    $('.left-block .img-box').one('inview', function (event, isInView) {
      if (isInView) {
        if (!$('.left-block .img-box').find('.slide-decor').hasClass('show')) {
          var slideCount = $('.left-block .swiper-slide').length;
          if (slideCount > 1) {
...
var _news = {
  ...
  swiper: function swiper() {
    var slider = _common.getMySwiper('top-news-slider');
    _common.bannerSlider(slider);
    slider.on('slideChangeTransitionStart', function () {
      _common.bannerSlider(slider);
    });
...
var _product_list = {
  ...
  filterMenu: function filterMenu() {
    var menuHeight = $('.filter-menu ul').innerHeight() + 2;
...
var _product_detail = {
  ...
  option: function option() {
    ...
    $('.option-bar .collect').on('click', function (e) {
      var category_h = $('.category-wrap').innerHeight();
      e.stopPropagation();
  ...
  imageZoom: function imageZoom() {
    var canZoom = $('body').attr('data-zoom') === '1' ? true : false;
    var zoomImgW = $('.section1 .block.right').innerWidth() - 30;
    var zoomImgH = $('.product-slider .img-box').innerHeight();
    $('.image-detail').css({
      'width': zoomImgW,
      'height': zoomImgH
    });
  ...
  swiper: function swiper() {
    ...
    var productSwiper = _common.getMySwiper('product-swiper');
  ...
  //色票收合
  colorCollapse: function colorCollapse() {
    var colorBox = $('.colorBox');
    var colorWrapWidth = $('.color-wrap').innerWidth();
    var oneLineShow = Math.floor(colorWrapWidth / colorBox.outerWidth(true));
    var totalLine = Math.ceil(colorBox.length / oneLineShow);
    if (totalLine > 1) {
  ...
  //選擇單位
  unitSelect: function unitSelect() {
    $('.unit-select .arrow-btn').on('click', function () {
      var select = $(this).parents('.unit-select');
      var mask = '<div class="mask" style="position: fixed;top: 0;left: 0;width: 100%;height: 100vh;background-color: rgba(0,0,0,0.6);z-index: 2;"></div>';
    ...
    $('.unit-radio input').on('change', function () {
      ...
      var smallUnit = $('.unit-select .item[data-index="1"]').attr('data-small-unit');
      var text = $(this).parents('.item').attr('data-text');
    ...
    var window_w = $(window).width();
    if (window_w <= 575) {
      var standard = $('.unit-select .item[data-index="0"]');
    ...
    $(window).on('resize', function () {
      ...
      if (newWidth > 575) {
        var _standard = $('.unit-select .item-wrap .item[data-index="0"]');
        $('.unit-select .item-wrap').prepend(_standard);
      } else {
        if (window_w === newWidth) {
          return false;
        }
        var selected = $('.unit-radio input:checked').parents('.item');
        $('.unit-select').removeClass('open');
  ...
  addCart: function addCart() {
    function showTips(el) {
      var text = $('.shop-control .add-order').attr('added-tips');
      var notice = "<div class='notice-wrapper'><div class='text'>" + text + "</div></div>";
      switch (el) {
        case 'nav:not(.sticky)':
    ...
    $('.shop-control .add-order').on('click', function (e) {
      ...
      if ($(window).outerWidth() > 1024) {
        ...
        var ball = '<div class="cart-ball" style="top: ' + mousePosition.y + 'px;left: ' + mousePosition.x + 'px"></div>';
        //重新產生新的小球
        $('.cart-ball').remove();
        $('body').append(ball);
        var last = $('.cart-ball');
        // 起始位置元素
        var startPoint = $('.cart-ball')[0];
        // 終點位置元素
        var endPoint = $('.fixed-left-function:not(.innerMenu) .icon-shop')[0];
        var myParabola = funParabola(startPoint, endPoint, {
          speed: 200, // 速度
  ...
  recommend: function recommend() {
    var window_w = $(window).width();
    var slideCounts = $('.recommend-product .swiper-slide').length;
    var options = {
      on: {
        init: function init() {
    ...
    var recommend = new Swiper('.recommend-product .swiper-container', options);
  },
  all: function all() {
...
var _product_search = {
  ...
  search: function search() {
    ...
    if ($('.keyword-search input').val() != '' || $('.search-catagory input:checked').length >= 1) {
      var selectMode = $('.search_sidebar .mode-item.selected').index();
  ...
  keywordCollapse: function keywordCollapse() {
...
var _qa = {
  ...
  anchorAndSwitchQA: function anchorAndSwitchQA() {
    if ($(window).outerWidth() > 1024) {
      ...
      $('.type-dropdown .dropdown ul > li').off().on('click', function () {
        var option = $(this).text();
        var target = $(this).attr('data-anchor-target');
        $('.type-dropdown .selected .text').text(option);
        if ($(window).outerWidth() <= 1024) {
          $(this).parents('.type-dropdown').removeClass('active');
          $(this).parents('.dropdown').slideUp(400);
...
var _shoppingCart = {
  ...
  productSelect: function productSelect() {
...
//結帳流程
var _checkout = {
  stepChange: function stepChange(type) {
    if ($('.step-content.step1').hasClass('empty')) return;
    var nowStepIndex = $('.step-wrap li.active').index() + 1;
    var stepTotal = $('.content .step-wrap > li').length;
    var prevStepName = function prevStepName() {
      var prevStep = $('.step-wrap li.active').prev('li');
      var leaveCheck = $('.step-change.prev').attr('data-leave-text');
      if (prevStep.find('.text .line1').length > 0) {
        if (prevStep.index() != 2) {
          ...
          if ($('.step-content.step3 .nav-tabs .tab-btn[data-page="form"]').hasClass('active') || $('.non-payable-block').length > 0) {
            var text = $('.footer .step-change.prev').attr('data-form-text');
            return text;
          } else if ($('.step-content.step3 .nav-tabs .tab-btn[data-page="payment"]').hasClass('active')) {
            var _text = $('.footer .step-change.prev').attr('data-pay-text');
            return _text;
          }
        }
      } else {
    ...
    var nextStepName = function nextStepName() {
      var nextStep = $('.step-wrap li.active').next('li');
      if (nextStep.find('.text .line1').length > 0) {
        return nextStep.find('.text .line1').text();
      } else {
        if ($('.step-content.step3 .nav-tabs .tab-btn[data-page="form"]').hasClass('active') || $('.non-payable-block').length > 0) {
          var text = $('.footer .step-change.next').attr('data-form-text');
          return text;
        } else if ($('.step-content.step3 .nav-tabs .tab-btn[data-page="payment"]').hasClass('active')) {
          var _text2 = $('.footer .step-change.next').attr('data-pay-text');
          return _text2;
        }
      }
    };
    ...
    if (typeof type == 'number') {
      ...
      var prevStepNum = nowStepIndex - 1 < 1 ? nowStepIndex : nowStepIndex - 1;
      var nextStepNum = nowStepIndex + 1 > stepTotal ? nowStepIndex : nowStepIndex + 1;
      switch (type) {
        case 'prev':
  ...
  form: function form() {
    ...
    $('.type-select .item').on('click', function () {
      var selected = $(this).attr('data-page');
      var selectedPayment = $('.payment-select .item.active').attr('data-page');
      $('.footer .step-change.next .step-btn').removeClass('disabled');
      $.anchor4({
...
var _about_history = {
  animate: function animate() {
    $('.top-motorcycle').playKeyframe(['motorcycle1 6s ease-in', 'motorcycle2 3s linear infinite']);
    $('.footer').one('inview', function (event, isInView) {
      if (isInView) {
        $('.bottom-motorcycle').playKeyframe(['motorcycle1 6s ease-in', 'motorcycle2 3s linear infinite']);
        setTimeout(function () {
          $('.bottom-motorcycle').addClass('reverse').playKeyframe('motorcycle3 4s ease-in forwards');
        }, 6000);
      }
    });
  },
...
var _contact = {
  ...
  resetForm: function resetForm() {
    $('.reset').on('click', function () {
      var select = $('.form-group .select-element');
...
//會員中心
var _member = {
  changeWelcomeBG: function changeWelcomeBG() {
    var el = $('.welcome');
    var desktopSrc = el.attr('data-desktop');
    var mobileSrc = el.attr('data-mobile');
    if ($(window).width() > 575) {
  ...
  getToday: function getToday() {
    ...
    var day = moment(today).format("Do");
    var year = moment(today).format("YY");
  ...
  dropdown: function dropdown() {
    ...
    $('#favorite .filter-type-select .dropdown ul > li').on('click', function () {
      var anotherSelect = $(this).parents('.item').siblings('.item');
      var anotherSelectDefault = anotherSelect.find('.categories-select').attr('data-default');
  ...
  //收藏篩選類別切換
  filterSwitch: function filterSwitch() {
...
var _orderlist = {
  all: function all() {
    var activeTab = $('.order-content-tab > li.active').attr('data-page');
...
var _gdpr = {
  switchStatus: function switchStatus() {
    $('.gdpr-menu > li .switch-input').on('click', function () {
      var gdprOption = $(this).parents('li').attr('class');
      var route1 = $(this).attr('data-ajax-route');
      var route2 = $(this).attr('data-ajax-route2');
  ...
  showNowStep: function showNowStep() {
    $('.step-wrap').each(function () {
      var nowStep = $(this).attr('data-now-step');
      $(this).find('.step-content.step' + nowStep).show();
    });
  },
  stepChange: function stepChange(tab, step) {
...
var _customized = {
  ...
  //備註表單開關
  remarkSwitch: function remarkSwitch() {
  ...
  remarkForm: function remarkForm() {
  ...
  mobileMenuSwitch: function mobileMenuSwitch() {
...
var readyFunction = {
  ...
  product_search: function product_search() {
    _product_search.all();
  },
  ...
  about_fabric: function about_fabric() {
    _about_common.all();
  },
  about_feature: function about_feature() {
    _about_common.all();
  },
  about_history: function about_history() {
    _about_history.all();
  },
  about_location: function about_location() {
    _about_location.all();
  },
  ...
  checkout_result: function checkout_result() {},
  customized: function customized() {
    _customized.all();
  }
};
